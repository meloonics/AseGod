...
const A = AseFile.MAGIC_NUMBER
enum {
	ERR_NO_VALID_FRAMES = A,
	ERR_SHIDDED_MY_PANT = A+1,
}

var _ase: AseFile = null :
	set(value):
		if value:
			_ase = value
			_status = _ase.error
...
var _tkt: AseExportTicket = null
var _status: Error = ERR_DOES_NOT_EXIST

#region Constructors
func _init(from_asefile: AseFile = null, with_ticket: AseExportTicket = null) -> void:
	_ase = from_asefile
	if _ase:
		if with_ticket:
			_tkt = with_ticket
		else:
			_tkt = AseExportTicket.new()
	else:
		_ase = create_minimal_asefile()
		if with_ticket:
			_tkt = with_ticket
		else:
			_tkt = AseExportTicket.new(AseExportTicket.ExportType.FILE_ASE)
...
func _load_ase(path: String) -> void:
	if FileAccess.file_exists(path):
		_ase = load(path) as AseFile
	else:
		_status = ERR_FILE_NOT_FOUND
		push_error("AseGod: Source file not found: ", path)


static func from_ticket(ticket: AseExportTicket) -> AseGod:
	return AseGod.new(null, ticket)

static func from_dict(dict: Dictionary) -> AseGod:
	return from_ticket(AseExportTicket.from_dict(dict))

static func from_path(path: String, export_type: AseExportTicket.ExportType = AseExportTicket.ExportType.RES_TEXTURE) -> AseGod:
	var tkt: AseExportTicket = AseExportTicket.new(export_type)
	tkt.source_path = path
	return from_ticket(tkt)

static func from_resource(res: Resource) -> AseGod:
...
static func create_minimal_asefile(
	...
	p_color_depth: AseFile.ColorDepth = AseFile.ColorDepth.RGBA
	) -> AseFile:
	...
	ase.flags = AseFile.HeaderFlags.OPACITY_VALID
	
	# Add frame
	var frame = AseFrame.new()
	frame.duration_ms = 100
	ase.frames.append(frame)
	
	# Add layer
	var layer = AseLayer.new()
	layer.layer_name = "Layer 1"
	layer.layer_type = AseLayer.LayerType.NORMAL
	layer.opacity = 255
	layer.flags = AseLayer.LayerFlags.VISIBLE
	layer.layer_index = 0
	...
	cel.width = 64
	cel.height = 64
	cel.cel_type = AseCel.CelType.COMPRESSED_IMAGE
...
func sprite_size(width: int, height: int = 0) -> AseGod:
...
func color_depth(depth: AseFile.ColorDepth = AseFile.ColorDepth.RGBA) -> AseGod:
	_ase.color_depth = depth
	return self

func add_layer(name: String = "", layer_type: AseLayer.LayerType = AseLayer.LayerType.NORMAL, opacity: int = 255) -> AseGod:
...
func add_frame(duration_ms: int = 100) -> AseGod:
	var frame = AseFrame.new()
	frame.duration_ms = duration_ms
	_ase.frames.append(frame)
	return self

func add_cel(frame_idx: int, layer_idx: int, x: int = 0, y: int = 0) -> AseGod:
	var cel = AseCel.new()
...
func add_tag(name: String, from_frame: int, to_frame: int, color: Color = Color.RED) -> AseGod:
	var tags_chunk: AseTags = null
	...
	if not tags_chunk:
		tags_chunk = AseTags.new()
		if _ase.frames.size() > 0:
			_ase.frames[0].chunks.append(tags_chunk)
	
	var tag = AseTag.new(from_frame, to_frame, 0, 0, name, color)
	tags_chunk.tags.append(tag)
	return self

func add_tileset(name: String, tile_width: int, tile_height: int, num_tiles: int) -> AseGod:
	var tileset = AseTileset.new()
	tileset.name = name
